...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-webserver; then
    ...
    buildah from --name nodebuilder-webserver -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi

echo "Build static UI files with node..."
buildah run nodebuilder-webserver sh -c "cd /usr/src/ui && yarn install && yarn build"
